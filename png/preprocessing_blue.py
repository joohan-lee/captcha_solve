...
os.chdir('C:\\BrityDefault\\qoo10_captcha_data\\png\\blue_img') #red_img 경로 지정
cur_path = os.getcwd()
...
for i in range(1,8):
    ...
    blue = utils.leave_blue(image.copy(),utils.BLUE)
    cv2.imshow('Image Gray', blue)
    cv2.waitKey(0)
    cv2.destroyAllWindows()

    converted_path = 'C:\\BrityDefault\\qoo10_captcha_data\\png\\blue_img\\converted'
    ...
    #이미지 합치기
    h, w, c = blue.shape
    background[50:50 + h, 150:150+w] = blue #blue 이미지를 background에 넣기
